...
src_filename = os.path.join(scriptdir, "data/p082_matrix.txt")
...
class Graph():
    ...
    def BFS(self, source):
        ...
        return [self.dist[(source, (t, self.end[0]))] for t in range(len(self.values[0]))]
...
#print([(t, len(Matrix[0])-1) for t in range(len(Matrix[0]))])
#print Matrix[0][79]
print(min(sum([myGraph.BFS((t,0)) for t in range(len(Matrix))], [])))
